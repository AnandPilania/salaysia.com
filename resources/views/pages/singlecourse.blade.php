	...
	<div class="container">
		...
		<ul id="myUL" class="clearfix">
			...
			<div class="col-sm-7">
              ...
              <h4><a class="clearfix" href="{{URL::to('courses/'.$listing->course_slug.'/'.$listing->id)}}" class="title">{{$listing->title}}</a>

</h4>
<p><i class="fa fa-bars" aria-hidden="true"></i>Course Type:- {{$listing->category}}</p>
<p>
					<i class="fa fa-graduation-cap " aria-hidden="true"></i>
                                    {{$listing->level}} 
				
					<i class="fa fa-clock-o" aria-hidden="true"></i> {{$listing->type}}
					<i class="fa fa-calendar" aria-hidden="true"></i> START.{{$listing->start_from}}
					<i class="fa fa-tag" aria-hidden="true"></i> ${{$listing->fees}} per year</p>
 
					
                                        <p><i class="fa fa-clock-o" aria-hidden="true"></i>Duration:- {{$listing->duration}} </p>
                                    {{$listing->description }}				
                                </div>
			</div>
			<div class="col-sm-3">
   @if(Auth::check())
				<div class="shortlist-block">
					<p class="short-icon"  onClick="divFunction({{$listing->id}})">Shortlist <i id="chnimg{{$listing->id}}" class="fa @if(\App\FavouriteCourse::where(array('listing_id'=>$listing->id,'user_id'=>Auth::user()->id))->count()>0) fa-heart @else fa-heart-o @endif " aria-hidden="true"></i></p>
				</div>
@endif
			</div>
