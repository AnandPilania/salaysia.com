@extends('app')
 @section('head_title', 'FeatureUniversities' .' | '.getcong('site_name') )    
@section('content')
...
<script type="text/javascript">
    $(document).ready(function(e) {
        $("#country").change(function(){
            ...
            $.ajax({
                type: "GET",
                url: "{{ URL::to('ajax_featureuniversities') }}/"+cat,
    ...
    $("#myInput").change(function(){
            ...
            $.ajax({
                type: "GET",
                url: "{{ URL::to('ajax_sortfeatureuniversities') }}/"+cat,
                success: function(result){
...
<section class="contact-form-section clearfix">
	<div class="container">
			...
			<h2 class="text-red">{{trans('home.featured_universities')}}</h2>
...
<option value="all">{{trans('home.all_country')}}</option>
...
<select id="myInput">
 <option value="all">{{trans('home.sorted_by')}}</option>
  ...
  <option value="Rankings">{{trans('home.rankings')}}Rankings</option>
  <option value="Popularty">{{trans('home.popularty')}}Popularty</option>
  <option value="Reviews">{{trans('home.reviews')}}Reviews</option>
	...
	@foreach($universities as $university)	
		...
		<h4><a href="{{URL::to('universities/'.$university->listing_slug.'/'.$university->id)}}" class="title">{{$university->title}}</a></h4>
			...
			<div class="col-sm-3">
				...
				<p class="short-icon"  onClick="divFunction({{$university->id}})">Shortlist <i id="chnimg{{$university->id}}" class="fa @if(\App\FavouriteUniversity::where(array('listing_id'=>$university->id,'user_id'=>Auth::user()->id))->count()>0) fa-heart @else fa-heart-o @endif " aria-hidden="true"></i></p>
				</div>
                             @endif
			</div>
			<button type="button" data-toggle="modal" data-target="#ContactInsitute<?php echo $i; ?>" class="contact-inst-btn pull-right">CONTACT INSTITUTION</button>
			</li>
	<!-----------contact institute-------------------->


  <!-- Modal -->
  <div class="modal fade" id="ContactInsitute<?php echo $i; ?>" role="dialog">
								...
								{!! Form::open(array('url' => 'contact_send','class'=>'','id'=>'search','role'=>'form')) !!}
									<div class="row">
										<div class="col-sm-6">
                                                                                    ...
                                                                                    <input  type="hidden" name="uni_id"  value="{{$university->id}}">
										</div>
										<div class="col-sm-6">
