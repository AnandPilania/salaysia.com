...
@section('head_keywords', $listing->keyword )
@section('head_description', $listing->description )
...
function divFunction(id){
 $.ajax({
          type: "GET",
          url: "{{ URL::to('ajax_favouriteuniversities') }}/"+id,
...
<section class="filter-universities-name contact-form-section clearfix">
	<div class="container">
		<div class="col-sm-9 ">
		<ul id="myUL" class="clearfix">
			<li class="clearfix">
			<div class="col-sm-2">
				<div class="university-img-logo single-page-unilogo">
				<img src="{{ URL::asset('upload/listings/'.$listing->featured_image.'-s.jpg') }}"/>
				</div>
			</div>
			<div class="col-sm-10">
			   @if(Auth::check())
				<div class="shortlist-block">
			<p class="short-icon"  onClick="divFunction({{$listing->id}})">Shortlist <i id="chnimg{{$listing->id}}" class="fa @if(\App\FavouriteUniversity::where(array('listing_id'=>$listing->id,'user_id'=>Auth::user()->id))->count()>0) fa-heart @else fa-heart-o @endif " aria-hidden="true"></i></p>
					
				</div>@endif  
				<div class="university-description">
              <h4><a class="clearfix" href="{{URL::to('universities/'.$listing->listing_slug.'/'.$listing->id)}}" class="title">{{$listing->title}}</a></h4>
                                    <p><i class="fa fa-map-marker " aria-hidden="true"></i>
                                    {{$listing->country}}<i class="fa fa-eye padding-left-icn" aria-hidden="true"></i>
                                    {{$listing->view_count}} VIEWS <i class="fa fa-comment padding-left-icn" aria-hidden="true"></i>
  @for($x = 0; $x < 5; $x++)
                    ...
                    @endfor
<strong>THE WORLD</strong> Ranking: {{$listing->ranking}}
...
<div class="w3-content" style="max-width:800px">
 @foreach($listing_gallery_images as $i => $gallery_img)
  <img class="mySlides" src="{{ URL::asset('upload/gallery/'.$gallery_img->image_name) }}" style="width:100%;hieght:400px">
 @endforeach

  <div class="w3-row-padding w3-section">
<?php $j=1 ; ?>
@foreach($listing_gallery_images as $i => $gallery_img)
    <div class="w3-col s4" style="float:left;padding:5px">
      <img width="150px" height="150px" class="demo w3-opacity"src="{{URL::asset('upload/gallery/'.$gallery_img->image_name) }}" onclick="currentDiv(<?php echo $j ; ?>)">
    </div>
<?php $j++ ; ?>
@endforeach  
  </div>
...
if(tabvalue=='no')
 {
    ...
    $("#Research").removeClass('in');
    $("#Research").removeClass('active');
    $("#Campus").removeClass('in');
    $("#Campus").removeClass('active');
    $("#Academic").removeClass('in');
    $("#Academic").removeClass('active');
    $("#Admissions").removeClass('in');
    $("#Admissions").removeClass('active');
    $("#Facilities").removeClass('in');
    $("#Facilities").removeClass('active');
    $("#Accommodation").removeClass('in');
    $("#Accommodation").removeClass('active');
 }
if(tabvalue=='Research')
 {
    $("#Research").addClass('in');
 ...
 if(tabvalue=='Admissions')
 {
    $("#Admissions").addClass('in');
 ...
 if(tabvalue=='Facilities and Services')
 {
    $("#Facilities").addClass('in');
...
if(tabvalue=='Accommodation')
 {
    $("#Accommodation").addClass('in');
    ...
    $("#Facilities").removeClass('active');
        ...
        <div class="profile-gallery" id="profile-gallery">
        <div class="row">
<h1>About {{$listing->title}}</h1>
<ul class="pull-right nav">
  <li class="addactive">
  <select onchange="mytab();" class="tabvalue">
    <option value="no">More</option>
    <option>Research</option>
    <option>Admissions</option>
    <option>Facilities and Services</option>
    <option>Accommodation</option>
</select>
</li>
</ul>
        <ul class="nav nav-tabs">
  <li class="active rmactive sactive"><a data-toggle="tab" href="#Welcome">Welcome</a></li>
  <li class="rmactive"><a data-toggle="tab" href="#Campus">Campus locations</a></li>
  <li class="rmactive"><a data-toggle="tab" href="#Academic">Academic Schools </a></li>
</ul>
...
<div class="tab-content">
  <div id="Welcome" class="tab-pane fade in active">
    <h3>Welcome</h3>
    <p> <?php if (isset($listing->welcome_content)){ ?> {!!html_entity_decode($listing->welcome_content) !!} <?php } ?></p>
  </div>
  <div id="Campus" class="tab-pane fade">
    <h3>Campus locations</h3>
    <p><?php if (isset($listing->campus_content)){ ?> {!!html_entity_decode($listing->campus_content) !!} <?php } ?></p>
  </div>
  <div id="Academic" class="tab-pane fade">
    <h3>Academic Schools</h3>
    <p><?php if (isset($listing->academic_content)){ ?>{!!html_entity_decode($listing->academic_content) !!} <?php } ?></p>
  </div>
   <div id="Research" class="tab-pane fade">
    <h3>Research</h3>
    <p> <?php if (isset($listing->research_content)){ ?>{!!html_entity_decode($listing->research_content) !!} <?php } ?></p>
  </div>
   <div id="Admissions" class="tab-pane fade">
    <h3>Admissions</h3>
    <p> <?php if (isset($listing->admissions_content)){ ?> {!!html_entity_decode($listing->admissions_content) !!} <?php } ?></p>
  </div>
   <div id="Facilities" class="tab-pane fade">
    <h3>Facilities and Services</h3>
    <p><?php if (isset($listing->facilities_content)){ ?> {!!html_entity_decode($listing->facilities_content) !!} <?php } ?></p>
  </div>
   <div id="Accommodation" class="tab-pane fade">
    <h3>Accommodation</h3>
    <p> <?php if (isset($listing->accommodation_content)){ ?>{!!html_entity_decode($listing->accommodation_content) !!} <?php } ?></p>
  </div>
</div>
...
@endif
            ...
            <div class="arrows-new-class">
                <a class="left carousel-control" href="#myCarousel-banner" role="button" data-slide="prev">
                    <span class="glyphicon " aria-hidden="true" ><i class="fa fa-angle-left" aria-hidden="true"></i>
</span>
                    <span class="sr-only">Previous</span>
                </a>
                <a class="right carousel-control" href="#myCarousel-banner" role="button" data-slide="next">
                    <span class="glyphicon " aria-hidden="true"><i class="fa fa-angle-right" aria-hidden="true"></i>
</span>
...
@if($listing->map_lat and $listing->map_long)
            ...
            <div class="col-md-12">
              <div class="map" id="googleMap" style="width:400px;height:380px;"></div>
            </div>
        @endif
 </div>
...
<section class="management-uni">
	<h1>See what you can study at {{$listing->title}}</h1>
         @foreach($courses as $course) 
            <div class="col-sm-6 home-img-gallery">
                <div class="brdr-red ">
                    <div class="prod">
                        <img src="{{ URL::asset('upload/listings/'.$course->featured_image.'-s.jpg') }}" class="img-responsive">
                        <div class="overlay text-center">
                            <div class="list-of-views">
                                        <p><i class="fa fa-bars" aria-hidden="true"></i> {{$course->category}}</p>
					<p><i class="fa fa-graduation-cap" aria-hidden="true"></i>{{$course->level}}</p>
					<p><i class="fa fa-clock-o" aria-hidden="true"></i> {{$course->type}}</p>
                                        <p><i class="fa fa-clock-o" aria-hidden="true"></i>Duration:- {{$course->duration}} </p>
					<p><i class="fa fa-calendar" aria-hidden="true"></i> START.{{$course->start_from}}</p>
					<p><i class="fa fa-tag" aria-hidden="true"></i> ${{$course->fees}} per year</p>
				</div>
                 ...
                 <div class="col-sm-3 noppading">
							...
							<div class="lookingfor-stdy-form">
								{!! Form::open(array('url' => 'contact_send','class'=>'','id'=>'search','role'=>'form')) !!}
                                    ...
                                    <input type="hidden" name="uni_id"  value="{{$listing->id}}">
										</div>
										<div class="col-sm-6">
...
<section class="red-icons-blogs bg-grey" style="display:none">
	<div class="container">
